...
void kernel_main() {
  ...
  const InterleavedAddrGen<true> in = {
    .bank_base_address = in_addr, .page_size = datum_size_bytes * TILE_WIDTH
  };
  const InterleavedAddrGen<true> out = {
    .bank_base_address = out_addr, .page_size = datum_size_bytes * TILE_WIDTH
  };
  ...
  for (uint32_t i = start_page_id; i < end_page_id; ++i) {
    ...
    for (uint32_t j = 0; j < TILE_WIDTH; ++j) {
      ...
      for (uint32_t k = 0; k < data_size / TILE_WIDTH; ++k) {
	uint64_t in_noc_addr = get_noc_addr(index[j] * data_size / TILE_WIDTH + k, in);
	uint64_t out_noc_addr = get_noc_addr((TILE_WIDTH * i + j) * data_size / TILE_WIDTH + k, out);
	noc_async_read(in_noc_addr, buffer_addr, TILE_WIDTH * datum_size_bytes);
	noc_async_read_barrier();
	noc_async_write(buffer_addr, out_noc_addr, TILE_WIDTH * datum_size_bytes);
